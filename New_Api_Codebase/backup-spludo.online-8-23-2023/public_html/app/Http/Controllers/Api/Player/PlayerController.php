...
class PlayerController extends Controller
{
    ...
    public function AddRewardCoins(Request $request)
    {
        ...
        // Find the player
        $player = Userdata::where('playerid', $request->playerid)->first();

        if ($player) {
            // Update the player's coin balance
            $player->totalcoin += $request->coins;
            $player->save();

            $response = ['notice' => 'Coins Successfully Added!', 'totalcoin' => $player->totalcoin];
            return response($response, 200);
        } else {
    ...
    public function AddWinCoins(Request $request)
    {
        // Validate the request
        $request->validate([
            'playerid' => 'required',
        ...
        if ($player) {
            // Update the player's coin balance
            $player->wincoin += $request->coins;
            $player->save();

            $response = ['notice' => 'Win Coins Successfully Added!', 'wincoin' => $player->wincoin];
            return response($response, 200);
        } else {
            $response = ['notice' => 'Player Not Found!'];
            return response($response, 404);
    ...
    public function DeductCoins(Request $request)
    {
        ...
        if ($player) {
            ...
            $response = ['notice' => 'Coins Successfully Deducted!', 'totalcoin' => $player->totalcoin];
            return response($response, 200);
        } else {
            $response = ['notice' => 'Player Not Found!'];
            return response($response, 404);
